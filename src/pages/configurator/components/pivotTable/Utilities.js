...
const aggregatorTemplates = {
  ...
  editable(formatter = usFmt) {
    return function([attr]) {
      return function(pivot, rowKey, colKey, i) {
        return {
          key: null,
          sum: 0,
          push(record) {
            let obj = record[attr];
            if (!isNaN(parseFloat(obj)) || obj === null) {
              this.sum += obj === null ? 0 : parseFloat(obj);
              if (i !== undefined && this.key === null) this.key = i;
              else this.key = -1;
            }
          },
          value() {
            const key = this.key;
            return key === null || key === -1 ? this.sum : { key, value: this.sum };
          },
          format: formatter,
          numInputs: typeof attr !== 'undefined' ? 0 : 1,
        };
...
class PivotData {
  ...
  arrSort(attrs) {
    // vlr - hack
     let specialId = attrs.indexOf("Номер столбца");
     //if (specialId < 0) specialId = attrs.indexOf("Номер строки");
    if (specialId >= 0) {
      return (a, b) => a[specialId] > b[specialId];
    }
    // vlr - hack
...
PivotData.forEachRecord = function(input, derivedAttributes, f) {
  ...
  if (typeof input === 'function') {
      ...
      return (() => {
        ...
        for (const i of Object.keys(input || {})) {
          ...
          if (i > 0) {
            ...
            result.push(addRecord(record, i));
          }
        }
        return result;
      })();
    ...
    return (() => {
      ...
      let i = 0;
      for (record of input) {
        result1.push(addRecord(record, i++));
      };
      return result1;
    })();
  }
